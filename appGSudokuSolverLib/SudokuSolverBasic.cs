...
internal class SudokuSolverBasic : ISudokuSolver
{
    ...
    {
        byte resultRowIndex = 9, resultColumnIndex = 9;
        int min = 10;
        for (byte i=0;i<9*9;i++)
        {
            ...
            if(availableNumbers.Count > 0 && availableNumbers.Count < min)
            {
                resultRowIndex = rowIndex;
                resultColumnIndex = columnIndex;
                min = availableNumbers.Count;
            }
        }
        return (resultColumnIndex, resultRowIndex);
    }
}
